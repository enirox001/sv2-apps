...
use stratum_apps::{
    ...
    stratum_core::bitcoin::{Address, Amount, Transaction, Txid, consensus::deserialize},
};
use tracing::warn;
...
impl BitcoinCore {
    ...
    ///
    /// Keeping construction separate from [`BitcoinCore::send_raw_transaction`] lets latency
    /// tests start their clock immediately before the `sendrawtransaction` RPC.
    pub fn create_signed_mempool_transaction(&self) -> Result<Transaction, String> {
        ...
        let transaction_bytes = hex::decode(&signed.hex)
